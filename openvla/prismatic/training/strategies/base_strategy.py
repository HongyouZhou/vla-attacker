        ...
        with tqdm(
            ...
            for batch in dataloader:
                ...
                with torch.autocast(
                    ...
                    # [Contract] self.vlm.forward() must automatically compute `loss` and return!
                    # Check if adversarial training before tokenization is enabled
                    if hasattr(self.vlm, 'adv_config') and getattr(self.vlm.adv_config, 'adv_training_action_pretokenization', False):
                        # Pass batch and batch_transform to support adversarial training before tokenization
                        output: CausalLMOutputWithPast = self.vlm(
                            input_ids=batch["input_ids"],
                            attention_mask=batch["attention_mask"],
                            pixel_values=batch["pixel_values"],
