...
class MaskStorage:
    ...
    def get_demo_dir(self, demo_info: DemoInfo) -> Path:
        ...
        return self.output_dir / demo_info.task_suite / demo_info.task_name / f"demo_{demo_info.demo_id}"
        ...
        with h5py.File(masks_path, 'w') as f:
            ...
            for frame_idx, result in results.items():
                ...
                if len(result.masks) > 0:
                    ...
                    frame_grp.create_dataset('masks', data=np.zeros((0, h, w), dtype=np.uint8))
                    frame_grp.create_dataset('object_ids', data=np.array([], dtype=np.int32))
                    frame_grp.create_dataset('bboxes', data=np.zeros((0, 4), dtype=np.float32))
                    frame_grp.create_dataset('scores', data=np.array([], dtype=np.float32))
