    ...
    def load_frames(
        ...
        with h5py.File(demo_info.hdf5_path, "r") as f:
            ...
            for i in range(start_frame, min(end_frame, num_frames)):
                # Images are stored as (T, H, W, C) in BGR format and upside-down
                # Convert BGR to RGB and flip vertically to get correct orientation
                img = images[i][:]
                if img.shape[-1] == 3:
                    img = img[::-1, :, ::-1].copy()  # Flip vertically + BGR to RGB
                yield i, img
...
if __name__ == "__main__":
    ...
    if demos:
        ...
        # Load first frame
        for idx, frame in loader.load_frames(
            demo, demo.available_views[0], end_frame=1
        ):
